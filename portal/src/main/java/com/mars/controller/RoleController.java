...
import com.mars.models.RolePaginationRequestParameters;
...
@RestController
@RequestMapping(value = APIUtilConstant.ROLE_API_END_POINT, produces = MediaType.APPLICATION_JSON_VALUE)
@Api(value = "Role APIs Detail")
public class RoleController
{
    ...
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = ErrorMessagesConstant.SUCCESS, response = Role.class),
    ...
    public Role getById(
            @ApiParam(name = "id", value = "role_id", required = true) @PathVariable("id") String id)
                    throws ResourceNotFoundException
    {
        return roleService.findById(id);
    }

    @ApiOperation(value = "Find all roles by pages", responseContainer = "List", response = Role.class)
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = ErrorMessagesConstant.SUCCESS, response = Role.class),
    ...
    public Page<Role> getAll(
            @Valid @ModelAttribute() RolePaginationRequestParameters paginationRequestParameters,
            BindingResult bindingResult)
    {
        ...
        return roleService.findAll(paginationRequestParameters.getPage(),
